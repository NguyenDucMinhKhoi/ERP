...
const InvoiceCreation = ({ student, onClose, onSubmit }) => {
  ...
  // Load all students once and filter client-side (like StudentList.jsx does)
  ...
  useEffect(() => {
    const loadAllStudents = async () => {
      try {
        console.log('📥 Loading all students for search...');
        const response = await crmService.getStudents();
        const students = response.results || response || [];
        console.log('👥 Loaded students:', students.length, students.slice(0, 2)); // Show first 2 for debug
  ...
  // Search for students (client-side filtering like StudentList.jsx)
  useEffect(() => {
    ...
    console.log('🔍 Filtering students for term:', searchTerm);
    ...
    console.log('🎯 Filtered results:', filteredStudents.length, filteredStudents);
    ...
    const loadCourses = async () => {
      ...
      try {
        ...
        const formattedCourses = coursesData.map(course => {
          ...
          return {
            id: course.id,
            name: course.ten || course.ten_khoa_hoc || course.name || 'Không có tên',
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'courseId' && formData.invoiceType === 'tuition' && !formData.customAmount) {
      ...
      console.log('💰 Auto-filling course fee:', { course, value, courseId: course?.id });
      if (course) {
        setFormData(prev => ({
          ...prev,
  ...
  const validateForm = () => {
    ...
    if (!selectedStudent) {
      ...
      newErrors.studentSearchTerm = 'Học viên đã đóng đủ học phí, không thể tạo hóa đơn';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const paymentPayload = {
        ...
        ghi_chu: formData.description + (formData.notes ? ` | ${formData.notes}` : ''),
      };
      ...
      const studentCodeFromId = String(selectedStudent.id).padStart(6, '0').substring(0, 6);
      ...
      const invoiceData = {
        ...
        courseName: selectedCourse?.name || selectedStudent?.khoahoc?.ten_khoa_hoc || 'Chưa có thông tin khóa học',
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white rounded-t-xl">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {selectedStudent ? `${selectedStudent.ten} (${selectedStudent.ma_hoc_vien})` : 'Tìm kiếm và chọn học viên để tạo hóa đơn'}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="relative student-search-container">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Tìm kiếm học viên * {!studentsLoaded && '(Đang tải danh sách học viên...)'}
                </label>
                <div className="relative">
                  <input
                    ...
                    placeholder={studentsLoaded ? "Nhập tên hoặc mã học viên..." : "Đang tải danh sách..."}
                    disabled={!!selectedStudent || !studentsLoaded}
                ...
                {console.log('🎯 Dropdown render conditions:', {
                  ...
                  searchResults: searchResults.slice(0, 3) // First 3 results for debugging
                })}
                {showSearchDropdown && searchResults.length > 0 && !selectedStudent && studentsLoaded && (
                    ...
                    {searchResults.map((student) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setShowSearchDropdown(false);
                          setErrors(prev => ({ ...prev, studentSearchTerm: '' }));
                        }}
                ...
                {/* No Results Message */}
                {showSearchDropdown && searchResults.length === 0 && formData.studentSearchTerm.trim().length >= 2 && studentsLoaded && !selectedStudent && (
                  <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg p-4 text-center text-gray-500">
                    Không tìm thấy học viên với từ khóa "{formData.studentSearchTerm}"
                    <br />
                    <small>Tổng số học viên có sẵn: {allStudents.length}</small>
                  </div>
                )}
                ...
                {selectedStudent && (
                  <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-md">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-blue-900">{selectedStudent.ten}</p>
                        <p className="text-xs text-blue-600">Mã: {selectedStudent.ma_hoc_vien}</p>
                        {selectedStudent.khoahoc?.ten_khoa_hoc && (
                          <p className="text-xs text-blue-600">Khóa: {selectedStudent.khoahoc.ten_khoa_hoc}</p>
                        )}
                        <p className="text-xs text-blue-600 mt-1">
                          Trạng thái: 
                          {selectedStudent.trang_thai_hoc_phi === 'chuadong' && ' Chưa đóng'}
                          {selectedStudent.trang_thai_hoc_phi === 'conno' && ' Còn nợ'}
                          {selectedStudent.trang_thai_hoc_phi === 'dadong' && ' Đã đóng đủ'}
                        </p>
                      </div>
                      <button
                        type="button"
                ...
                {errors.studentSearchTerm && (
                  <p className="mt-1 text-sm text-red-600">{errors.studentSearchTerm}</p>
                )}
              </div>
              ...
              <div>
                ...
                >
                  <option value="">
                    {coursesLoading ? 'Đang tải khóa học...' : 'Chọn khóa học'}
                  </option>
                  {courses.map((course) => (
